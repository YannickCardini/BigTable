...
public class TasteOfRiak {
	...
	// TODO : modifier le nom du dossier
	...
	private static void addMultiplePerson(List<Person> listPers, RiakClient client)
			throws InterruptedException {
		...
		{
			bucket = new Namespace("person", pers.getId());
			if (pers.getBirthday() != null && pers.getBirthday().length() > 0) {
				loc = new Location(bucket, "birthday");
				ro = new RiakObject();
				ro.setValue(BinaryValue.create(pers.getBirthday()));
	...
	private static void addMultipleFeedback(List<Feedback> listFeed, RiakClient client)
			throws InterruptedException {
		...
		{	bucket = new Namespace("feedback", feed.getAsin() + feed.getPersonId());// Combo asin +personId
			if (feed.getFeedback() != null && feed.getFeedback().length() > 0) {
				loc = new Location(bucket, "feed");
				ro = new RiakObject();
				ro.setValue(BinaryValue.create(feed.getFeedback()));
	...
	private static void addMultipleProduct(List<Product> listProd, RiakClient client)
			throws InterruptedException {

		Location productLocation;
		RiakObject ro;
		StoreValue storeProductOp;
		Namespace productBucket;

		for(Product prod:listProd)
		{
			productBucket = new Namespace("product", prod.getAsin());
			if (prod.getPrice() != null && prod.getPrice().length() > 0) {
				productLocation = new Location(productBucket, "price");
				ro = new RiakObject();
				ro.setValue(BinaryValue.create(prod.getPrice()));
	...
	private static void addMultipleVendor(List<Vendor> listVend, RiakClient client)
			throws InterruptedException
	{
		Location vendorLocation;
		RiakObject ro;
		...
		for(Vendor vend:listVend)
		{	bucket = new Namespace("vendor", vend.getVendor());
			if (vend.getCountry() != null && vend.getCountry().length() > 0) {
				vendorLocation = new Location(bucket, "country");
				ro = new RiakObject();
				ro.setValue(BinaryValue.create(vend.getCountry()));
	...
	private static void addMultipleInvoice(List<Order> listInv, RiakClient client)
			throws InterruptedException
	{
		Location invoiceLocation;
		RiakObject ro;
		...
		for(Order inv: listInv)
		{	bucket = new Namespace("invoice", inv.getOrderId());
			if (inv.getPersonId() != null && inv.getPersonId().length() > 0) {
				invoiceLocation = new Location(bucket, "personId");
				ro = new RiakObject();
				ro.setValue(BinaryValue.create(inv.getPersonId()));
	...
	private static void addMultipleOrder(List<Order> listOrd, RiakClient client)
			throws InterruptedException
	{
		Location vendorLocation;
		RiakObject ro;
		...
		{	bucket = new Namespace("order", ord.getOrderId());
			if (ord.getPersonId() != null && ord.getPersonId().length() > 0) {
				vendorLocation = new Location(bucket, "personId");
				ro = new RiakObject();
				ro.setValue(BinaryValue.create(ord.getPersonId()));
...
//			storeProduct(PATH + "Product/Product.csv", PATH + "Product/BrandByProduct.csv", client); //
           ...
           //Ajout du Product
			...
			updateProduct("productTest","title","titletest2",client);

			//suppression d'un bucket
			deleteProduct("productTest",client);
